using LPR381_Windows_Project.Misc;
using System;
...
namespace LPR381_Windows_Project.Algorithms
{
    public class SimplexSolver : ISolver
    {
        public void Solve(LinearModel model, out string output)
        {
            ...
            sb.AppendLine("Primal Simplex Iterations:");
            ...
            for (int i = 0; i < m; i++)
            {
                if (model.Constraints[i].Relation != "<=")
                    sb.AppendLine($"Warning: Constraint {i + 1} uses {model.Constraints[i].Relation}; only <= supported.");
                for (int j = 0; j < n; j++)
